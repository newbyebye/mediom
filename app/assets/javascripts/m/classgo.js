...
$(function() {
    ...
    CG.HomePullDownRefresh = CG.PullDownRefresh.extend({
        ...
        loadData: function(skip, limit, callback){
            CG.PostController.get('/v1/posts?skip='+skip+'&limit='+limit+'', function(err, data){
                callback(err, data);
            });
        },

        getCount: function(callback){
            CG.PostController.get('/v1/posts/count', function(err, content){
        ...
        showData: function(item){
            ...
            html += '<img src="'+ item.User.Avatar +'">';
            html += '<h2>' + item.Title + '</h2>';
            html += '<p class="ui-li-aside"><strong>'+$.timeago(item.CreatedAt)+'</strong></p></a><hr></li>';
            return html;
        }
    });
    ...
    CG.MyClassPullDownRefresh = CG.PullDownRefresh.extend({
        ...
        loadData: function(skip, limit, callback){
            CG.PostController.get('/v1/posts/owner?skip='+skip+'&limit='+limit+'', function(err, data){
                callback(err, data);
            });
        },

        getCount: function(callback){
            CG.PostController.get('/v1/posts/owner/count', function(err, content){
                callback(err, content);
            });
        },
        ...
        showData: function(item){
            var html = '<li data-icon="false"><a href="#pageDetail?id='+ item.Id +'">';
            html += '<h2>' + item.Title + '</h2>';
            html += '<span class="ui-li-count">'+$.timeago(item.CreatedAt)+'</span></a><hr></li>'
            return html;
        }
    });

    CG.GamePullDownRefresh = CG.PullDownRefresh.extend({
        //costomer define
        listSelector:"div.game-page ul.ui-listview",
        loadData: function(skip, limit, callback){
            var postId = window.sessionStorage.getItem("postId");
            CG.PostController.get('/v1/posts/' + postId + '/games?skip='+skip+'&limit='+limit, function(err, data){
                callback(err, data);
            });
        },
        ...
        getCount: function(callback){
            var postId = window.sessionStorage.getItem("postId");
            CG.PostController.get('/v1/posts/' + postId + '/games/count', function(err, content){
                callback(err, content);
            });
        },

        showData: function(item){
            var html = '<li data-icon="false"><a href="javascript:void(0)" onclick="viewActivateGame('+ item.Id +','+item.Status+')">';
            var name = item.GameTemplate.Name;
            if (item.GameTemplate.Subname){
               name += " - " + item.GameTemplate.Subname;
            }
            ...
            html += '<p class="ui-li-aside"><strong>'+$.timeago(item.CreatedAt)+'</strong></p>';
            html += '</a><hr></li>';
            return html;
        }
    });
...
$(function(){
	...
	CG.MyController = CG.SingleObjController.extend({
		objName: "./v1/users/" +window.sessionStorage.getItem('userId')
	});
...
function savePost(){
    ...
    if (id == ''){ 
        ...
        CG.PostController.put('/v1/posts/'+id, data, function(err, result){
            viewHomeDetail(id);
            back();
        });
    }
...
function exportPost(){
    var postId = window.sessionStorage.getItem("postId");
    window.location='/v1/posts/'+postId+'/report'+"?token="+window.sessionStorage.getItem("token");
}

function exportGame(){
    var postId = window.sessionStorage.getItem("postId");
    window.location='/v1/posts/'+postId+'/gamereport'+"?token="+window.sessionStorage.getItem("token");
}

var fileForm = new Object();
function checkFileSize(fileObj) {
    $.ajaxFileUpload({  
            url:"./v1/posts/"+window.sessionStorage.getItem("postId")+"/upload?token="+window.sessionStorage.getItem("token"),  
            secureuri:false,
  ...
  function newPost(){
    ...
    function loadRegistData(){
        CG.PostController.get('/v1/posts/register', function(err, data){
            if (err) {
                return;
            }
    ...
    function loadTodayLesson(lesson) {
        if (lesson && lesson.Status == 1) {
            $('#detail_lesson_id').val(lesson.Id);
            getMySignInStatus();
    ...
    function getMySignInStatus() {
        ...
        CG.PostController.get('/v1/posts/lesson/'+$('#detail_lesson_id').val()+'/sign', function(err, data){
            if (!err){
                if (data.id != 0) {
	...
	function viewHomeDetail(id){
        ...
        CG.PostController.get('/v1/posts/'+id, function(err, data){
            $('#detail_id').val(id);
            $('#detail_author').text(filterXSS(data.User.Fullname + '-'+data.User.Profession));
            $('#detail_photo').attr('src', filterXSS(data.User.Avatar));
            $('#detail_title').text(filterXSS(data.Title));
            $('#detail_sum').text(filterXSS("" + data.RegisterCount));
            loadTodayLesson(data.Lesson)
            ...
            if (checkCurrentUserId(data.UserId)){
                $('#detail_owner').show();
                $('#detail_edit').show();
                $('#detail_other').hide();
            }
    ...
    function registerClass() {
        CG.PostController.post('/v1/posts/'+$('#detail_id').val()+'/register', {}, function(err, data){
            if (!err){
    ...
    function signIn() {
        ...
        wx.getLocation({
            ...
            success: function (res) {
                ...
                CG.PostController.post('/v1/posts/lesson/'+$('#detail_lesson_id').val()+'/sign', data, function(err, data){
                    if (!err){
    ...
    function startSignIn() {
        ...
        wx.getLocation({
            ...
            success: function (res) {
                ...
                CG.PostController.post('/v1/posts/'+$('#detail_id').val()+'/lesson', data, function(err, data){
                    if (!err){
    ...
    function deletePost(){
        ...
        if (confirm("删除是不可恢复的，你确认要删除吗？")){
            CG.PostController.remove('/v1/posts/'+id, function(err, data){
                if (!err){
...
(function pullPagePullImplementation($) {
  ...
  function wxinit(){
        ...
        var noncestr= CryptoJS.lib.WordArray.random(16).toString();
  ...
  $(document).on("pagebeforechange", function(e, f){
    ...
    if (hash === "#pageDetail"){
      if (search){
        ...
        CG.PostController.get("/v1/posts/"+window.sessionStorage.getItem("postId")+"/minireport", function(err, result){
